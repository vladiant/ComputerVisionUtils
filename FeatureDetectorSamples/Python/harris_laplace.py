...
while True:
    ...
    current_corners_thresh = cv.getTrackbarPos('Corners Threshold', 'HarrisLaplace') / 100
    current_dog_thresh = cv.getTrackbarPos('DOG Threshold', 'HarrisLaplace') / 1000

    # Initiate HarrisLaplace object with default values
    fast = cv.xfeatures2d.HarrisLaplaceFeatureDetector_create(numOctaves=current_num_octaves,
