...
constexpr auto noiseName = "Gaussian";
...
cv::Mat gaussian(const cv::Mat& image, float mean, float var,
                 unsigned int seed) {
  gen.seed(seed);
...
void update(int, void* data) {
  ...
  const auto noised = gaussian(input_img, mean, var, 42);
